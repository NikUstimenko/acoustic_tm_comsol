...
Adapted for acoustic T-matrices of 3D objects by Nikita Ustimenko, 2024.
'''
...
l_in = np.int64(model.evaluate('l_in', dataset=dsname))
l_out = np.int64(model.evaluate('l_out', dataset=dsname))
m_in = np.int64(model.evaluate('m_in', dataset=dsname))
m_out = np.int64(model.evaluate('m_out', dataset=dsname))
...
with open('tmatrix_coeffs.txt', 'w') as f:
    ...
    for i in range(len(l_in)):
        f.write(f'{l_in[i]}   {l_out[i]}   {m_in[i]}   {m_out[i]}   {freq[i]}   {ap[i]}\n')
